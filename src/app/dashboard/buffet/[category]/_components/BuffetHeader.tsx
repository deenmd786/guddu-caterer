...
const BuffetHeader: React.FC<BuffetHeaderProps> = ({ title, discount }) => {
  return (
    <div className="flex justify-between items-center bg-white p-4 rounded-lg shadow-md hover:shadow-lg transition-all duration-300">
      ...
      {discount && (
        <span className="inline-flex items-center text-base font-semibold text-red-600 bg-red-100 px-4 pr-[.3rem] py-2 rounded-full shadow-md hover:shadow-lg transition-all duration-300">
          {discount}% Off
        </span>
      )}
    </div>
